...
int main(){
	...
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++){
			if (tracks[i][j] == '<' || tracks[i][j] == '>' || tracks[i][j] == '^' || tracks[i][j] == 'v'){
				...
				currentCart.lastPos = currentCart.pos;
			...
			for (int j = 0; j < width; j++){
				...
				if (tracks[i][j] == '<' || tracks[i][j] == '>' || tracks[i][j] == '^' || tracks[i][j] == 'v'){ //found a cart
					...
					if (currentCart->tileUnder == '<' || currentCart->tileUnder == '>' || currentCart->tileUnder == '^' || currentCart->tileUnder == 'v'){
						std::vector<cart> tempCarts;
						char replacementTile;
						for (auto c : carts){
							if (c.pos.x != currentCart->pos.x || c.pos.y != currentCart->pos.y){
								tempCarts.push_back(c);
							}
							if(c.pos.x == currentCart->pos.x && c.pos.y == currentCart->pos.y){
								visited.push_back(c.id);
								std::cout << "removed: " << c.pos.y << "," << c.pos.x << "  " << c.tileUnder<< std::endl;
								if(c.tileUnder != '<' && c.tileUnder != '>' && c.tileUnder != '^' && c.tileUnder != 'v')
									replacementTile = c.tileUnder;
							}
						}
						tracks[currentCart->pos.x][currentCart->pos.y] = replacementTile;
						carts = tempCarts;
						std::cout << carts.size() << " left" << std::endl <<  std::endl;
	...
	std::cout << carts[0].pos.y << "," << carts[0].pos.x;
}
