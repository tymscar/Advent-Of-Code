...
const playMonkeyAction = (monkeys: Monkey[], _: Monkey, currIdx: number): Monkey[] => {
	...
	const inCommon = monkeys.reduce((total: bigint, curr: Monkey) => total * curr.divNum, BigInt(1));
	
	const toThrow: [number, bigint][] = curr.items.map(item => {
		const newWorry: bigint = curr.operation(item) % inCommon;
...
const topTwoInspected = itemsInspected.sort((a: number, b:number) => b - a).slice(0, 2);
