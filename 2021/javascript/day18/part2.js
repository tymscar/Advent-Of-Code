...
for(let i=0; i<input.length; i++){
	for(let j=0; j<input.length; j++){
		if(i!==j){
			numberPairs.push([JSON.parse(input[i]),JSON.parse(input[j])]);
		}
	}
}

const reducedTrees = numberPairs.map(number => createTree(0,number).reduce());
const magnitudes = reducedTrees.map(tree => tree.magnitude());
const answer = Math.max(...magnitudes);
